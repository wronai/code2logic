#!/usr/bin/env python3
"""
Ultimate fix for map object errors in advanced data analyzer.
"""
...
class UltimateAdvancedDataAnalyzer:
    ...
    def run_all_analyses(self) -> List[Dict]:
        ...
        print("🔍 Running 10 Ultimate Advanced Analyses on Hybrid Export Data...")
        print("=" * 60)
...
"""
    ...
    # Helper methods - ALL MAP OBJECT ERRORS FIXED
  ...
  • Stwórz shared context dla wspólnych danych"""
    ...
    def generate_summary_report(self):
        ...
        print(f"\n📊 ULTIMATE SUMMARY REPORT - {len(self.analysis_results)} Analyses")
        print("=" * 60)
        ...
        print(f"\n💾 Saved {len(self.analysis_results)} LLM queries to 'ultimate_llm_refactoring_queries.yaml'")
    ...
    def _save_llm_queries(self):
        ...
        output_path = self.hybrid_path / 'ultimate_llm_refactoring_queries.yaml'
...
def main():
    ...
    if not Path(hybrid_path).exists():
        print("❌ Hybrid export not found. Trying output...")
        hybrid_path = 'output'
        if not Path(hybrid_path).exists():
            print("❌ No hybrid export found. Run code2flow first.")
            return
