...
poetry run code2logic code2logic -f hybrid -o project.c2l.hybrid.yaml --with-schema
poetry run code2logic code2logic -f toon -o project.c2l.toon --with-schema

echo
echo "=== ROZMIARY WYGENEROWANYCH PLIKÓW (kB) ==="

yaml_bytes=$(stat -c%s project.c2l.yaml 2>/dev/null || echo 0)
hybrid_bytes=$(stat -c%s project.c2l.hybrid.yaml 2>/dev/null || echo 0)
toon_bytes=$(stat -c%s project.c2l.toon 2>/dev/null || echo 0)
...
echo "YAML:   ${yaml_kb} kB (project.c2l.yaml)"
echo "Hybrid: ${hybrid_kb} kB (project.c2l.hybrid.yaml)"
echo "TOON:   ${toon_kb} kB (project.c2l.toon)"
