...
### Flow 6: _register_builtin_actions
```
_register_builtin_actions [registry.ActionRegistry]
```

### Flow 7: _execute_desktop_plan_step
```
_execute_desktop_plan_step [pipeline_runner_desktop.DesktopExecutionMixin]
```

### Flow 8: _generate_rule_based_canvas_plan
```
_generate_rule_based_canvas_plan [automation.action_planner.ActionPlanner]
```
...
### automation.action_planner._parse_llm_response
> Parse LLM JSON response into ActionPlan.
- **Output to**: re.sub, re.sub, ActionPlan, raw.strip, json.loads
